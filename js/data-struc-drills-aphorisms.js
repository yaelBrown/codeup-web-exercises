...
//  Arrays begin and end with square brackets. '[]'. Objects begin and end with curly-braces '{}'. Multiple items in this array are separted by commas.
...
var lastQuote = [
    {
        q: aphorisms[aphorisms.length - 1].quote,
        a: aphorisms[aphorisms.length - 1].author,
    }
];
...
//      Would be able to do the same thing with less lines of code, espcially if you need to log a larger number of quotes.
...
let formatQuote = (aph) => {
    console.log(aph.quote + ". -" + aph.author);
}

// formatQuote(aphorisms[12]);
...
let getRandomQuote = (aph) => {
  let randomNum = Math.floor(Math.random() * aph.length);

  formatQuote(aph[randomNum]);
//   console.log(aph[randomNum].quote + ". -" + aph[randomNum]);
};
...
// how to do this with forEach loop

// anonQuotes(aphorisms);
...
// console.log(aphorisms.indexOf(aphorisms[34]));

let everyOddQuote = (aph) => {
  var temp = [aph.length];
  var count = 0;
